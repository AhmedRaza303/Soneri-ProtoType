...
interface ModifyValueScreenProps {
  systemValue: SystemValue;
  onBack: () => void;
  onUpdateValue: (updatedValue: SystemValue) => void;
...
export const ModifyValueScreen: React.FC<ModifyValueScreenProps> = ({
  systemValue,
  onBack,
  onUpdateValue,
  existingParentCodes = DEFAULT_PARENT_CODES,
}) => {
  const initialParent = systemValue.parentCode || systemValue.category || 'AccountType';
  const initialName = systemValue.valueName || systemValue.name || '';
  const initialOrder = systemValue.displayOrder ? String(systemValue.displayOrder) : '1';
  const initialDesc = systemValue.description || '';

  const [parentCode, setParentCode] = useState(initialParent);
  ...
  const parentOptions = Array.from(
    new Set([...DEFAULT_PARENT_CODES, ...existingParentCodes, initialParent])
  ).filter(Boolean);

  useEffect(() => {
    setParentCode(systemValue.parentCode || systemValue.category || 'AccountType');
    setValueName(systemValue.valueName || systemValue.name || '');
    setDisplayOrder(systemValue.displayOrder ? String(systemValue.displayOrder) : '1');
    setDescription(systemValue.description || '');
  }, [systemValue]);
  ...
  return (
    <div className="min-h-full pb-24 bg-[#f8fafc] text-slate-800">
      <AppBar
        title="Modify Value"
        subtitle={`Editing: ${systemValue.valueName || systemValue.name}`}
        showBack
        onBack={onBack}
      />
        ...
        <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200/90 shadow-xs flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Update value parameters and display order.
            </p>
          </div>
          <button
            type="button"
        ...
        >
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              <label
                htmlFor="mod-parent-code-select"
          ...
          <div className="space-y-1.5">
            <label
              htmlFor="mod-value-name-input"
              className="text-xs font-bold text-slate-700 block"
            >
              Value Name <span className="text-rose-500">*</span>
            </label>
            <input
              id="mod-value-name-input"
              type="text"
              value={valueName}
          ...
          <div className="space-y-1.5">
            <label
              htmlFor="mod-display-order-input"
              className="text-xs font-bold text-slate-700 block"
            >
              Display Order <span className="text-rose-500">*</span>
            </label>
            <input
              id="mod-display-order-input"
              type="text"
              value={displayOrder}
          ...
          <div className="space-y-1.5">
            <label
              htmlFor="mod-description-textarea"
              className="text-xs font-bold text-slate-700 block"
            >
              Description
            </label>
            <textarea
              id="mod-description-textarea"
              rows={4}
              value={description}
